...
function ReadCounter(comport:string): integer;
 ...
 if 0 = 0 then begin
     ...
     if result=INVALID_HANDLE_VALUE then   exit
      ...
      // If not EscapeCommFunction(result,5) then RaiseLastwin32error;
       ...
       if not BuildCommDCB('baud=9600 parity=N data=8 stop=1', dcb) then RaiseLastwin32error;
                          // dcb.Flags := 17;
...
function ReadCounter(comport:string): integer;
  ...
  begin
    try
      rs485 := openport(comport);
      if (rs485 <> INVALID_HANDLE_VALUE) then closehandle(rs485);
      rs485 :=  INVALID_HANDLE_VALUE;
    except
    end;
    if (rs485 <> INVALID_HANDLE_VALUE) then closehandle(rs485);
    rs485 :=  INVALID_HANDLE_VALUE;
 ...
 if 0 = 0 then begin
     ...
     if result=INVALID_HANDLE_VALUE then   exit
      ...
      // If not EscapeCommFunction(result,5) then RaiseLastwin32error;
       ...
       fillchar( dcb, sizeof(dcb), 0);
       dcb.DCBlength := sizeof(dcb);
       dcbparm:='baud='+baud+' parity=N data=8 stop=1';
